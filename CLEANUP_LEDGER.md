...
- openspec/specs/eval/generators/gsm8k | scanned | 8 / 5 | Uncovered: "stream items are valid" (no v1 stream-schema validator exists to run), "full dataset default" (no test generates with default problem_count=None). Partial: "truth on side channel only" (tests assert no-answer leak but not probe_id/task_id). Covered: "deterministic stream", "chance rate reported", "subset mode" (test_each_problem_yields_one_observe_then_one_probe, problem_count=5 -> 10 items), "observe events contain word problems" (cycle 36: test_observe_events_render_word_problems, renamed from test_observe_events_have_text_payload, now asserts rendered Observe has alphabetic text + a digit + multiple words), "probe events ask for numerical answers" (cycle 36: test_probe_events_ask_for_numerical_answers, renamed from test_probe_events_have_query_string, now asserts rendered Probe ends with "?" and names answer + problem). Note: 9 of 11 covers comments still reference pre-restructure requirement names - stale linkage; the 2 strengthened tests' covers comments now name the current scenarios
...
- openspec/specs/eval/reproduction-gate | scanned | 6 / 1 | CORRECTION (cycle 32): the reproduction gate runner DOES exist as scripts/run_gate.py (complete: 5 seeds, tolerance/qualitative/ordering/reproducibility criteria, gate_report.json); it is a script with no direct test file, so it counts 0 in the covered tally. Only the three baselines (eval/baselines/{naive,ewc,replay}.py) and tests/gate/test_gate_smoke.py have test coverage. Covered: "exact repeat" (tests/run/test_resume.py test_two_runs_with_same_seed_produce_same_probe_log asserts same-config+seed -> identical probe log; the deterministic-CUDA-recording half is only in the stage0-gate token baseline, not here). Partial: "all three methods run on the same stream" (test_gate_smoke.py runs naive/EWC/replay on identical tiny synthetic split-classify streams, not the class-incremental Split-MNIST the GIVEN requires). Uncovered: "tolerance is declared" (no per-method tolerance written anywhere), "five seeds" (no 5-seed reproduction run; eval/gate/cycling_sweep.py is Gate 8.4 cycling sweep, a different gate, 3 seeds), "ordering check" (no replay>EWC/naive comparison), "paper code as subject" (no paper-code wrapping)
...
- openspec/specs/train/alternating-cycle | scanned | 32 / 28 | hysteresis control (4), traversal (4), comparable metrics (2), evaluation (3), resume (3), standalone compat (2), throttled output (10), graph export (2). Covered via tests/test_alternating_{scheduler,config,checkpoint,evaluation}.py + test_run_alternating.py + test_run_eggroll.py + test_run_training.py + test_plot_training.py. Partial (4): "Switch from Eggroll to gradient training" + "Return to Eggroll" (shared params + separate optimizer state tested by test_main_builds_shared_production_run_and_executes_schedule / test_typed_builder_captures_real_model_optimizer_and_rng_state / test_compatible_checkpoint_restores_model_optimizers_and_every_rng_state, but no end-to-end switch test asserts the handoff of final-step parameter values); "Default training progress interval" (default log_every=50 asserted in test_alternating_command_defaults, interval emission only verified at N=3/4); "Epoch boundary progress records" (epoch+partial_phase record asserted in test_small_injected_run, pure epoch-only boundary record never asserted). "Non-progress output remains independent" covered cycle 34 by test_progress_filter_leaves_non_structured_output_untouched (structured stream = JSON records only, human stream survives the filter)
- openspec/specs/train/eggroll-execution | scanned | 12 / 11 | factorized perturbations (3), low-rank assembly (3), optimized/reference equivalence (3), CUDA benchmark (3). Covered via tests/test_eggroll_{factorized,perturbations,updates,step_equivalence,resume_equivalence}.py + test_benchmark_eggroll.py. Partial (1): "Performance gate passes" - the two rejection conditions are asserted (test_performance_gate_rejects_insufficient_speedup, test_performance_gate_rejects_peak_memory_increase) but no test asserts the pass branch (speedup >= 3x and memory <= reference) itself
