...
from train.stage0_checkpoint import CheckpointContainerError
from test_stage0_checkpoint_resume import _metadata, _plain
...
def _state(mode: str):
    ...
    optimizer = torch.optim.Adam(parameters.values(), lr=1e-4)
...
def test_compatible_standalone_resume_restores_model_and_optimizer(
    ...
    restored_optimizer = torch.optim.Adam(parameters.values(), lr=0.9)
