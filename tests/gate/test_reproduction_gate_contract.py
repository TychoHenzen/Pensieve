...
def _summary(means: dict[str, float], seed_count: int = 5) -> dict:
    ...
    return {
        method: {"mean": mean, "accuracies": [mean] * seed_count}
...
def test_seed_count_criterion_rejects_fewer_than_five_seeds():
    four_seed_summary = _summary(
        {"naive": 20.0, "ewc": 20.0, "replay": 85.0}, seed_count=4
    )
    four_seed_criteria = gate._evaluate_criteria(
        four_seed_summary, [0, 1, 2, 3], _reproducibility()
    )
    assert four_seed_criteria["criterion_4_seed_count"] is False

    five_seed_summary = _summary({"naive": 20.0, "ewc": 20.0, "replay": 85.0})
    five_seed_criteria = gate._evaluate_criteria(
        five_seed_summary, [0, 1, 2, 3, 4], _reproducibility()
    )
...
def test_seed_count_criterion_requires_five_accuracies_per_method():
    ...
    criteria = gate._evaluate_criteria(
        short_summary, [0, 1, 2, 3, 4], _reproducibility()
    )
