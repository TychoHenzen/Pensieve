...
- **THEN** the input uses Qwen's official chat template and continues the exact answer-only assistant prefill
...
Default files MUST be `gate_results/asdiv_a_qwen/token_cot.json`, `latent_eval.json`, and `gate_report.json`. Each file MUST be at most 16 MiB before parsing and use schema version 2 with maximum nesting depth 8, at most 3,000 collection items, duplicate JSON keys and non-finite JSON numbers rejected, and strings subject to their field limits. Token schema MUST contain `{schema_version, identity, items, correct, total}`. Every token item MUST be `{item_id, completion, generated_token_count, hit_token_limit, prediction, target, correct}`. `completion` MUST preserve the raw decoded suffix after the assistant prefill. Latent schema MUST contain `{schema_version, identity, checkpoint_sha256, seeds, runs}` where every run is `{seed, items, correct, total}` and every latent item is `{item_id, prediction, target, correct}`. The identity MUST contain the complete dataset selection identity, model and tokenizer asset identities, exact prompt and assistant prefill, `rendered_inputs_sha256`, scoring grammar/version, full token and latent generation settings, Python and dependency versions, device, float32 dtype, eager attention implementation, `CUBLAS_WORKSPACE_CONFIG`, deterministic-backend settings, slot count, latent-step count, and tap tuple index. `rendered_inputs_sha256` MUST hash canonical JSON over the ordered list of `{item_id, input_ids}` objects. A cached result MUST be reused only when its typed identity matches the request, ids are unique and complete, and every stored prediction, correctness value, count, and aggregate recomputes from the raw completion plus canonical target. Latent identity MUST also match the checkpoint SHA-256 and exact seeds. Writers MUST fsync a temporary file in the destination directory and atomically replace the destination only after complete serialization. Gate commands MUST accept only local result paths.
