...
An oracle subject MUST store every observed key-value pair and answer every probe correctly for any fact it has seen. [REQUIRED]
...
An oracle MUST remember only the most recent item. It answers correctly if the probe asks about the last thing it saw. It answers wrong otherwise. [REQUIRED]
...
An oracle MUST learn the current task perfectly and destroy the previous task's knowledge on every task switch. After a `Boundary(TASK_SWITCH)`, it forgets the task that just ended. [REQUIRED]
...
An oracle MUST learn from probes. It reads the truth answer during `answer()` and stores it. It exists solely to test that probe isolation catches it. [REQUIRED]
