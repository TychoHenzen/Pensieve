...
- [ ] 5.1 Add a versioned checkpoint payload containing named model state, both optimizers, schedule position, configuration, held-out selection, metrics, and Python and PyTorch random states.
...
- [ ] 6.1 Add `train.run_alternating` with five epochs and 500 phase steps by default, distinct learning-rate flags, existing Eggroll controls, held-out size, logging, save directory, and resume options.
