...
- [x] 8.1 Run all focused EGGROLL, Stage 0 training, alternating scheduler, checkpoint, alignment-search, and stability-gate tests with `.\.venv\Scripts\python.exe -m pytest -q -p no:cacheprovider`.
<!-- status: completed -->
- [x] 8.2 Run the complete project test suite and strict OpenSpec validation. Repair regressions without weakening scenarios or tolerances.
<!-- status: completed -->
